...
std::list<Edge>& GraphWithEdge::get_adj(int v) {
    return adj[v];
}

void GraphWithEdge::add_vertex(int v){
    // O(V)
    if(!has_vertex(v)){
        adj.insert(std::next(adj.begin(), v), std::list<Edge>());
    }
    else{
...
void GraphWithEdge::remove_vertex(int v){
    // O(V . E)
    if(has_vertex(v)){
        // remove vertex
...
bool GraphWithEdge::has_vertex(int v){
    // O(1)
    return adj.size() > v;
}

std::size_t GraphWithEdge::size(){
    // O(1)
    return adj.size();
}

void GraphWithEdge::add_edge(int v1, int v2, int weight){
    // O(E)
    if(has_vertex(v1) && has_vertex(v2)){
        if(!has_edge(v1, v2)){
            auto src_vertex = adj.begin();
            std::advance(src_vertex, v1);
            src_vertex->push_back({v1, v2, weight});
        }
        else{
...
void GraphWithEdge::add_undirected_edge(int v1, int v2, int weight){
    // O(E)
    add_edge(v1,v2,weight);
    add_edge(v2,v1,weight);
}

void GraphWithEdge::remove_edge(int v1, int v2){
    // O(E)
    if(has_vertex(v1) && has_vertex(v2)){
        auto src_vertex = adj.begin();
        std::advance(src_vertex, v1);
        auto edge_it = std::find_if(src_vertex->begin(), src_vertex->end(), [&](const auto& edge){
            return edge.dst == v2;
        });
...
bool GraphWithEdge::has_edge(int v1, int v2){
    // O(E)
    if(has_vertex(v1) && has_vertex(v2)){
        auto src_vertex = adj.begin();
        std::advance(src_vertex, v1);
...
int GraphWithEdge::get_edge_len(int v1, int v2){
    // O(E)
    if(has_vertex(v1) && has_vertex(v2)){
        auto src_vertex = adj.begin();
        std::advance(src_vertex, v1);
